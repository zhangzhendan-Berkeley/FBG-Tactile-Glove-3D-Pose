...
def main(config_path: str, run_dir_arg: str | None = None):
    ...
    run_dir = run_dir_arg or os.path.join("runs", "mamba_coarse_only")
    ...
    va_data = {
        "files": cfg["data"].get("val_files", cfg["data"]["test_files"]),
        "schema_file": cfg["data"].get("schema_file"),
        "window_size": cfg["data"]["window_size"],
        "window_stride": cfg["data"]["window_stride"],
        "sensor_scale": cfg["data"].get("sensor_scale", 1024.0),
    ...
    te_data = dict(va_data)
    te_data["files"] = cfg["data"]["test_files"]
    _, te_loader = make_loader(
        te_data,
        bs=cfg["train"]["batch_size"],
        shuffle=False,
        num_workers=cfg["train"].get("num_workers", 0),
    ...
    test_metrics = evaluate(model, te_loader, device, supervision=supervision, ref_frame=ref_frame, scaler=scaler)
