...
#endif
#define SERIAL_PORT_RATE	256000




#ifndef BUF_SIZE1
# define BUF_SIZE1 197
#endif
Serial serial;
...
int index = 0;
int findhead = 0;
double maxdat = 16777215;
double halfdat = 8388608;
double mindat = 65536;
double litdat = 256;
double Dataout_0 = 0, Dataout_1 = 0, Dataout_2 = 0, Dataout_3 = 0;
byte* head;
...
int main(int argc, char* argv[])
{
	// print version info
	SerialSetup();
	//thread t(readSensor, DevIndex); // 开启力传感器读取线程
	thread Serialrun(serialread);
	unsigned char ver[4];
	const char* szFile = "non-cut-1-9287.txt";
	if (argc > 3)
		szFile = argv[3];
	...
	/*ofstream outfile;
	outfile.open("record.txt", ios::binary | ios::trunc | ios::in | ios::out);
	*/
	// Connect to Motive
...
void NATNET_CALLCONV DataHandler(sFrameOfMocapData* data, void* pUserData)
{
	...
	for (i = 0; i < data->nLabeledMarkers; i++)
	{
		...
		//temp = USB3GetAi(DevIndex, 10, Ai, 100);// 开始读取数据。		
		...
		printf(" [ModelID=%d, MarkerID=%d, Occluded=%d, PCSolved=%d, ModelSolved=%d] [pos=%6.5f,%6.5f,%6.5f]\n",
			modelID, markerID, bOccluded, bPCSolved, bModelSolved, marker.x, marker.y, marker.z);

		//_WriteFrame(g_outputFile, data);
		fprintf(g_outputFile, "%d; %6.5f; %6.5f; %6.5f; % .6f； % .6f; % .6f; % .6f\n", markerID, marker.x, marker.y, marker.z, Dataout_0, Dataout_1, Dataout_2, Dataout_3);
...
void serialread()
{
	while (isRun)
	{
		...
		if (serial.available() >= res)//
		{
			res = serial.read(buf, res);

			if (*(buf + findhead) != 0x7c)
			{
				head = find(buf, buf + res, 0x7c);
				findhead = std::distance(buf, head);
			}

			if (findhead > 181 && findhead < 197)
			{
				serial.read(buf, findhead);
				serial.read(buf, res);
				head = find(buf, buf + res, 0x7c);
				findhead = std::distance(buf, head);
			}

			Dataout_0 = 5 * ((buf[findhead + 3] * mindat + buf[findhead + 4] * litdat + buf[findhead + 5] - halfdat) / maxdat);

			Dataout_1 = 5 * ((buf[findhead + 6] * mindat + buf[findhead + 7] * litdat + buf[findhead + 8] - halfdat) / maxdat);

			Dataout_2 = 5 * ((buf[findhead + 9] * mindat + buf[findhead + 10] * litdat + buf[findhead + 11] - halfdat) / maxdat);

			Dataout_3 = 5 * ((buf[findhead + 12] * mindat + buf[findhead + 13] * litdat + buf[findhead + 14] - halfdat) / maxdat);
