...
def motion_capture_thread():
    ...
    while not stop_event.is_set():
        ...
        if frame is None:
            continue
        markers_info = [(rigid.Id, rigid.X, rigid.Y, rigid.Z, rigid.qx, rigid.qy, rigid.qz, rigid.qw) for rigid in frame.rigidBodys]
        with data_lock:
...
def data_write_thread():
    with open("sync_data-250819-test3.txt", "w", buffering=1) as f:  # line-buffered，写一行就落盘
            ...
            with data_lock:
                if latest_motion_data and latest_arduino_data:
                    ...
                    for marker in latest_motion_data:
                        marker_str_list.extend([str(marker[0]), str(marker[1]), str(marker[2]), str(marker[3]), str(marker[4]), str(marker[5]), str(marker[6]), str(marker[7])])
                    arduino_str_list = [str(v) for v in latest_arduino_data]
                    line = ",".join(marker_str_list + arduino_str_list) + "\n"
                    f.write(line)
                    print("Motion markers:")
                    for marker in latest_motion_data:
                        print(f"ID: {marker[0]}, X: {marker[1]}, Y: {marker[2]}, Z: {marker[3]}, QX: {marker[4]}, QY: {marker[5]}, QZ: {marker[6]}, QW: {marker[7]}")
