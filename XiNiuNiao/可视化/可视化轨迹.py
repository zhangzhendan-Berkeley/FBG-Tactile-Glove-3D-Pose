...
class TrajectoryViewer:
    def __init__(self, root, gt_txt, pred_txt):
        ...
        # Tick label offset settings (for Y axis)
        self.y_tick_offset = 0.05
    ...
    def show_instructions(self):
        ...
        • Y-axis shows only 3 ticks to avoid overlap
        • Adjust Y轴刻度数字偏移避免与X轴重叠
    ...
    def show_about(self):
        ...
        - Optimized tick spacing to prevent overlap
        - Adjustable Y轴刻度数字偏移避免与X轴重叠
    ...
    def _compute_axis_limits(self):
        ...
        # Store data bounds for auto adjustment
        self.data_bounds = {
            'min': mn,
            'max': mx,
            'padding': padding
    ...
    def _set_axis_ticks(self, ax):
        """Set axis ticks with appropriate spacing to avoid overlap"""
        # X轴刻度
        x_ticks = np.linspace(self.xlim[0], self.xlim[1], self.num_ticks['x'])
        ax.set_xticks(x_ticks)
        ax.set_xticklabels([f"{tick:.1f}" for tick in x_ticks], fontsize=12, fontname='Times New Roman')

        # Y轴刻度
        y_ticks = np.linspace(self.ylim[0], self.ylim[1], self.num_ticks['y'])
        ax.set_yticks(y_ticks)
        y_labels = [f"{tick:.1f}" for tick in y_ticks]
        ax.set_yticklabels(y_labels, fontsize=12, fontname='Times New Roman')

        # 调整Y轴刻度数字的位置，向右偏移避免与X轴数字重叠
        for label in ax.get_yticklabels():
            # 获取当前文本位置
            pos = label.get_position()
            # 设置新的位置：X方向增加偏移量
            label.set_position((pos[0] + self.y_tick_offset, pos[1]))
            # 设置文本对齐方式
            label.set_ha('left')
            label.set_va('center')

        # Z轴刻度
        z_ticks = np.linspace(self.zlim[0], self.zlim[1], self.num_ticks['z'])
        ax.set_zticks(z_ticks)
        ax.set_zticklabels([f"{tick:.1f}" for tick in z_ticks], fontsize=12, fontname='Times New Roman')
    ...
    def _set_axis_labels(self, ax):
        ...
        ax.set_xlabel('X (mm)', fontsize=14, fontname='Times New Roman',
                      labelpad=self.axis_label_offset['x'])
        ax.set_ylabel('Y (mm)', fontsize=14, fontname='Times New Roman',
                      labelpad=self.axis_label_offset['y'])
        ax.set_zlabel('Z (mm)', fontsize=14, fontname='Times New Roman',
                      labelpad=self.axis_label_offset['z'])

    def _build_ui(self):
        """Build user interface"""
        # Create main container with scrollbar for control panel
    ...
    def _build_control_panel(self, parent):
        ...
        ttk.Label(ytick_offset_frame, text="调整Y轴刻度数字位置避免与X轴重叠", font=('Arial', 9, 'italic')).pack(pady=2)
        ...
        self.ytick_offset_var = tk.DoubleVar(value=0.05)
        # 移除resolution参数，使用Scale的默认行为
        ...
        self.ytick_offset_label = ttk.Label(offset_slider_frame, text="0.05", width=6)
        self.ytick_offset_label.pack(side=tk.LEFT, padx=5)

        # Axis Label Offset Control
        ...
        self.x_offset_var = tk.IntVar(value=15)
        self.x_offset_scale = ttk.Scale(
            x_offset_frame, from_=5, to=40, orient=tk.HORIZONTAL,
        ...
        self.x_offset_label = ttk.Label(x_offset_frame, text="15", width=4)
        ...
        self.y_offset_var = tk.IntVar(value=25)
        self.y_offset_scale = ttk.Scale(
            y_offset_frame, from_=5, to=50, orient=tk.HORIZONTAL,
        ...
        self.y_offset_label = ttk.Label(y_offset_frame, text="25", width=4)
        ...
        self.z_offset_var = tk.IntVar(value=15)
        self.z_offset_scale = ttk.Scale(
            z_offset_frame, from_=5, to=40, orient=tk.HORIZONTAL,
        ...
        self.z_offset_label = ttk.Label(z_offset_frame, text="15", width=4)
        ...
        self.z_tick_var = tk.IntVar(value=5)
        self.z_tick_scale = ttk.Scale(
            z_tick_frame, from_=3, to=10, orient=tk.HORIZONTAL,
            variable=self.z_tick_var, command=self.on_tick_change
        )
        ...
        self.legend_pos_combo = ttk.Combobox(pos_frame, textvariable=self.legend_pos_var,
        ...
        self.elev_label = ttk.Label(elev_frame, text="20°", width=6)
        ...
        self.azim_label = ttk.Label(azim_frame, text="-45°", width=6)
    ...
    def on_tick_change(self, val):
        ...
        self.num_ticks['z'] = int(self.z_tick_var.get())
    ...
    def reset_view(self):
        ...
        self.elev_label.config(text="20°")
        self.azim_label.config(text="-45°")
        self.xlim = self.original_limits['x']
        self.ylim = self.original_limits['y']
        self.zlim = self.original_limits['z']
        self.update_plot()
    ...
    def save_figure(self, file_format):
        ...
        if not file_path:
            return
        try:
            save_fig = Figure(figsize=(12, 9), dpi=600)
            save_fig.subplots_adjust(left=0.1, right=0.9, top=0.95, bottom=0.08)
            plt.rcParams['font.family'] = 'Times New Roman'
            ...
            save_ax.set_zticks(z_ticks)
            save_ax.set_xticklabels([f"{tick:.1f}" for tick in x_ticks], fontsize=24, fontname='Times New Roman')
            save_ax.set_yticklabels([f"{tick:.1f}" for tick in y_ticks], fontsize=24, fontname='Times New Roman')
            save_ax.set_zticklabels([f"{tick:.1f}" for tick in z_ticks], fontsize=24, fontname='Times New Roman')

            # 设置Y轴刻度数字偏移
            y_labels = save_ax.get_yticklabels()
            ...
            # 设置轴标签偏移
            save_ax.set_xlabel('X (mm)', fontsize=24, fontname='Times New Roman', labelpad=self.axis_label_offset['x'])
            save_ax.set_ylabel('Y (mm)', fontsize=24, fontname='Times New Roman', labelpad=self.axis_label_offset['y'])
            save_ax.set_zlabel('Z (mm)', fontsize=24, fontname='Times New Roman', labelpad=self.axis_label_offset['z'])

            legend = save_ax.legend(loc=self.legend_location, fontsize=20)
            ...
            try:
                ...
                pass
            save_fig.savefig(file_path, format=file_format, dpi=600)
            plt.close(save_fig)
    ...
    def update_plot(self):
        ...
        # Set axis ticks with optimized spacing
        self._set_axis_ticks(self.ax)

        # Set axis labels with optimized offsets
        self._set_axis_labels(self.ax)

        # Add legend with user settings
